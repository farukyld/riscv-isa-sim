...
static void commit_log_print_insn(processor_t *p, reg_t pc, insn_t insn)
{
  ...
  for (auto item : load) {
    ...
    if (unlikely(log_vaddr_paddr))
    {
      ...
      fprintf(log_file, " ");
      // !!! mem_read'ler icin paddr'i log_file'a yaz.
    ...
    {
      // !!! xlen 64, adres her zaman 64 bit olarak yaziliyor.
      commit_log_print_value(log_file, xlen, std::get<0>(item));
    }
  }
  ...
  for (auto item : store) {
    ...
    fprintf(log_file, " ");
    // bellege yazilan degerin byte cinsinden genisligi item denilen tuple'in 3. (0 index'li sistemde) elemani
    // bit cinsine cevrilip (<< 3 ) commit_log_print_value fonksiyonuna gonderiliyor.
